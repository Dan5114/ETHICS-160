...
class AppProfileController extends Controller
{
    ...
    public function store(Request $request): RedirectResponse|JsonResponse
    {
        ...
        try {
            ...
            $this->get_research($request->research_title);
    ...
    public function get_research($title)
    {
        try {
             // Use the dynamic connection to query the external database
            $externalResearch = DB::connection('mysql_external')
            ->table('tbl_research')
            ->where('research_title', 'like', '%' . $title . '%')
            ->first();

            if ($externalResearch) {
                DB::connection('mysql_external')->table('research_rero_sched')->insert([
                    'research_id' => $externalResearch->id,
                    'date_submitted' => now(),
                    'created_at' => now(),
                    'updated_at' => now(),
                ]);
